...
function ENT:Think()	
	...
	if (self.Pod and IsValid(self.Pod) and self.Pod.GetPassenger) then  --Determines whether stuff comes from vehicle or entity
		self.GyroDriver, self.entorpod  = self.Pod:GetPassenger(1), self.Pod	
	else
		self.entorpod = self	
	end 
	...
	if self.SystemOn or (joystick and joystick.Get(self.CPL, "gyro_launch")) then
		...
		if !self.gravtrigger then
			self.AllGyroConstraints = constraint.GetAllWeldedEntities( self )
...
function ENT:FreezeMotion()  --Freezes all constrained entities
	local constrainedents = constraint.GetAllWeldedEntities( self )
