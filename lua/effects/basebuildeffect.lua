...
local model =self.ParentEntity
local mn, mx = model:GetRenderBounds()
local Bottom = model:GetPos() + mn;
local Top = model:GetPos() + mx;

local Mult = (Top-Bottom):Length()
local rad = Mult

self.EndPos = LerpVector( 0.5, Bottom, Top )

if(math.random(1,7)==5)then
	local a = math.random(9999)
	local b = math.random(1,180)
	local X = math.sin(b)*math.sin(a)*rad
	local Y = math.sin(b)*math.cos(a)*rad
	local Z = math.cos(b)*rad
	...
	if particle then
		...
		particle:SetGravity(NORM(self.EndPos-(Pos+self.EndPos))*(1.5*Mult))
...
local Bottom = model:GetPos()+ model:GetUp()*model:BoundingRadius()*2;
local Top = model:GetPos();
